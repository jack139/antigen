...
​		使用深度学习模型进行试剂盒结果判读，通常分为两个步骤：（1）从图片中检测试剂盒标线区域的位置；（2）对标线结果进行判读。例如，下图中方框部分即为标线区域。
...
3. 试剂盒可能因为手持、塑料袋包装等物品进行部分遮挡；
4. 试剂盒旁可能会出现文字说明、身份证、户口簿等说明性文件；
5. 不同生产厂商的试剂盒正面布局、文字、图案等也有一定的差异。

​		因此，数据集构建的过程，可以分布两步：（1）自动化生成试剂盒图片；（2）图片数据的标注。
...
1. 定义：
   ...
   （1）图片集合characters，包含可收集到的不同厂商的试剂盒封面图片，均为正常视角；

   <img src="images/2.png" alt="1" style="zoom: 40%;" />

   （2）图片集合backgrounds，包含各种背景图片，例如：纯色背景、各类材质的图片、各类复杂背景图片等；

   <img src="images/3.png" alt="1" style="zoom: 40%;" />

   （3）图片集合objects，包含各种可能与试剂盒同时存在的物品图片，例如：手持的造型、身份证件、说明卡片等；
...
4. 随机生成图片B的缩放比例，（其中randint返回两个参数之间范围的随机整数）
...
8. 随机生成图片A2在图片B2上的位置坐标(x, y)，方法如下：（其中abs()为取绝对值）
    ```
...
12. 随机产生D相对与A2的位置D_pos，其中D_pos可能的取值为```["left", "right", "above", "below"]```（即，D可以在A2的左侧、右侧、上面、下面）
...
16. 将图片E进行高斯模糊处理，高斯模糊的系数随机选取```[0, 1, 2]```（其中0即为不进行模糊处理），生成图片F

17. 保存图片F，即为一个随机生成的训练集图片

18. 重复2-17的步骤，直到完成指定数量的图片生成



​		使用上述步骤就可以批量自动生成具备泛化能力的训练数据集图片，例如下图所示。

<img src="images/5.png" alt="1" style="zoom: 40%;" />



### 数据标注的方法

​		在完成图片生成后，为进行模型训练，还需要对数据集图片进行标注，以训练模型可以检测到试剂盒标线区域的位置。
...
1. 定义：正常视角下，试剂盒标线区域的对角坐标点为A、B两点，其中A为正常视角左上角顶点，B为正常视角右下角顶点。不论试剂盒如何旋转，AB两点也同时旋转。如下图所示：
...
4. 对A、B的坐标进行归一化处理：
   ...
   至此，图片C可以用四个[0,1]区间的浮点数进行标注：```(Xa2, Ya2, Xb2, Yb2)```



​		由上述定义可知，因为A、B两点随着试剂盒的旋转同时旋转，同时已知试剂盒旋转角度为```[0, 90, 180, 270]```四个取值，因此试剂盒旋转的角度，可以根据A、B坐标的相对位置进行推定。
...
​		当前，用于目标检测的模型非常丰富。从检测步骤上可分为一阶定位模型和二阶定位模型。从模型结构上，基本均由骨干网络、颈部网络、头部网络三部分组成。因为试剂盒的位置检测相对简单，使用一阶定位方法就已足够。骨干网络可以使用Vgg、MobileNet、ResNet等常见的卷积网络，颈部网络可以使用特征金字塔（FPN）网络（如果为简化参数量，也可以不使用颈部网络），头部网络使用普通的多层全连接网络。

​		为使用前述定义的训练数据集，需要对头部网络进行改进。在传统的目标检测网络中，坐标回归一般使用relu激活函数。在前述训练集的数据标注中，因为进行了数据归一化处理，因此，头部网络的最后一层需要使用sigmod激活函数进行输出，保证输出的数据在[0,1]范围内，而且只需要输出4个数据。

<img src="network.jpg" alt="1" style="zoom: 80%;" />

​		我们实际使用的网络模型示意图如上，骨干网络使用ResNet50，颈部使用FPN，头部使用若干全连接层，最后输出4个使用sigmod激活的数据。



### 损失函数的设计

​		损失函数（loss function）是用来估量模型的预测值与真实值的不一致程度，对模型训练时能否正常收敛起到关键作用。结合试剂盒位置检测的目标和坐标数值回归的目标，损失函数（box_loss）的设计由两部分组成：IoU损失和MSE均方差损失。其中，IoU为预测区域与真实区域相交面积和合并面积的比值。MSE均方差为预测值和真实值之间距离的平方。

​		这里需要特别设计的是IoU损失函数。由前述数据集中标注信息的定义，A、B两点的坐标是随着试剂盒一通旋转的，因此，相较于普通的IoU计算，不能假设A、B两点相对位置是固定的，即IoU的计算不能依赖A、B点的相对位置。



**模型训练的损失函数的计算如下：**

1. 设真实值为y_true为一个四个元素的数列，用y_true[n]表示，n=0,1,2,3

   设预测值为y_pred为一个四个元素的数列，用y_pred[n]表示，n=0,1,2,3
...
3. 由上，可得预测值与真实值的相交面积 intersection

    ```
    intersection = max(w3, 0) * max(h3, 0)
    ```

4. 可得预测值与真实值的相合面积 union
...
6. 设mse()为均方差损失函数
7. 进一步，可得试剂盒位置预测模型的损失函数 box_loss
...
​		至此，使用改进设计的损失函数box_loss，可以针对前述训练数据集进行模型训练，保证模型的收敛，实现试剂盒标线区域检测的功能。
