...
def get_model(input_size = (256,256,3), weights='imagenet'):
    ...
    p4_output = Lambda(lambda x: x + p5_output)(p4_output_1) #(None, 8, 8)
    p4_output = UpSampling2D((2,2))(p4_output) #(None, 16, 16)
    p3_output = Lambda(lambda x: x + p4_output)(p3_output_1) #(None, 16, 16)

    # 红线箭头
    p5_output = Lambda(lambda x: x + UpSampling2D((2,2))(p5_output))(p3_output_1) #(None, 16, 16)
    ...
    n4_output = Lambda(lambda x: x + p4_output)(n3_output_3)

    n4_output_3 = Conv2D(256, 3, strides=1, activation='relu', padding="same")(n4_output)
    n5_output = Lambda(lambda x: x + p5_output)(n4_output_3)

    # 绿线箭头
    n5_output = Lambda(lambda x: x + p5_output)(p3_output_1)
