...
def iou_loss(y_true, y_pred, left_right=0):
    ...
    AoG = K.abs(K.transpose(y_true)[2+offset] - K.transpose(y_true)[0+offset] + penalty) \
        * K.abs(K.transpose(y_true)[3+offset] - K.transpose(y_true)[1+offset] + penalty)
    
    # AOP = Area of Predicted box
    AoP = K.abs(K.transpose(y_pred)[2+offset] - K.transpose(y_pred)[0+offset] + penalty) \
        * K.abs(K.transpose(y_pred)[3+offset] - K.transpose(y_pred)[1+offset] + penalty)
